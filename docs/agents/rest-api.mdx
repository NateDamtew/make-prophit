...
During the testnet phase the read API is intentionally open — there are no hard
rate limits yet. **Don't build on the absence of limits**: when we move toward
mainnet we'll introduce per-IP limits for anonymous traffic and higher per-agent
